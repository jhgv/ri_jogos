...
package crawler.crawlercommons;
...
import crawler.crawlercommons.SimpleRobotRules.RobotRulesMode;
...
public class SimpleRobotRulesParser extends BaseRobotsParser {
	...
	public BaseRobotRules parseContent(String url, byte[] content, String contentType, String robotNames) {
		...
		boolean isHtmlType = ((contentType != null)
				&& contentType.toLowerCase(Locale.getDefault()).startsWith("text/html"));
		...
		if (isHtmlType || SIMPLE_HTML_PATTERN.matcher(contentAsStr).find()) {
			if (!USER_AGENT_PATTERN.matcher(contentAsStr).find()) {
				System.out.println("Found non-robots.txt HTML file: " + url);
				return new SimpleRobotRules(RobotRulesMode.ALLOW_ALL);
			} else {
				// We'll try to strip out HTML tags below.
				if (isHtmlType) {
					System.out.println("HTML content type returned for robots.txt file: " + url);
				} else {
					System.out.println("Found HTML in robots.txt file: " + url);
				}
		...
		while (lineParser.hasMoreTokens()) {
			...
			case MISSING:
				reportWarning(String.format(Locale.getDefault(), "Unknown line in robots.txt file (size %d): %s",
						content.length, line), url);
				parseState.setFinishedAgentFields(true);
				break;
		...
		if (result.getCrawlDelay() > MAX_CRAWL_DELAY) {
			...
			System.out.println("Crawl delay exceeds max value - so disallowing all URLs: " + url);
	...
	private void reportWarning(String msg, String url) {
		...
		if (_numWarnings == 1) {
			System.out.println("Problem processing robots.txt for " + url);
		}
